...
int invn_mag_init(inv_imu_device_t *s)
{
	...
	INV_MSG(INV_MSG_LEVEL_INFO, "Reseting Ict1531x");
	...
	if (rc != INV_ERROR_SUCCESS) {
		INV_MSG(INV_MSG_LEVEL_ERROR, "Failed to initialize ict1531x.");
		return rc;
	}

	/* Check WHOAMI */
	INV_MSG(INV_MSG_LEVEL_VERBOSE, "Check Ict1531x whoami value");

	rc = inv_ict1531x_get_whoami(&mag_dev, &data);
	if (rc != INV_ERROR_SUCCESS) {
		INV_MSG(INV_MSG_LEVEL_ERROR, "Failed to read Ict1531x whoami value.");
		return rc;
	}

	if (data != ICT1531X_WHOAMI) {
		INV_MSG(INV_MSG_LEVEL_ERROR, "Bad WHOAMI value for mag device. Got 0x%02x", data);
		return INV_ERROR;
	}

	if (rc < 0)
		INV_MSG(INV_MSG_LEVEL_WARNING, "Error while setting up Mag device");
	else
		INV_MSG(INV_MSG_LEVEL_INFO, "Magnetometer successfully initialized");
