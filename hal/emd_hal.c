...
int emd_hal_gpio_init(unsigned int int_num, emd_gpio_cb_t cb)
{
    ...
    /* For INT1, we rely on poll in the main loop.
     * libgpiod line is already requested by emd_hal_init(). */
    return 0;
}
                 ...
                 const char *gpio_chip, unsigned int gpio_line)
{
    ...
    /* INT1: active low → request falling edge.
     * But DTS says IRQ_TYPE_LEVEL_LOW, and the MCU uses rising edge
     * after polarity config. We request rising edge to match MCU. */
    if (gpiod_line_request_rising_edge_events(g_gpio_line, "emd-gaf") < 0) {
        fprintf(stderr, "EMD_HAL: gpiod request event failed\n");
        gpiod_chip_close(g_gpio_chip);
        close(g_i2c_fd);
        return -1;
    }
